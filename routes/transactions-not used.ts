...
//   try {
//     await pgNeonClient.query(`CREATE TABLE IF NOT EXISTS HotelInfo ( id serial PRIMARY KEY, numbers text,
//       emailRec text NULL, displayNumber text NULL )`)
//     const result = await pgNeonClient.query('SELECT emailRec from HotelInfo where id=1')
...
//         await pgNeonClient.query(`CREATE TABLE IF NOT EXISTS PaidToHotel ( id serial PRIMARY KEY, transferRef text,
//           amount text, timestamp text, transactionId text)`)
//         await pgNeonClient.query(`INSERT INTO PaidToHotel ( transferRef, amount, timestamp, transactionId) VALUES
//           ('${transferRef}', '${amount.toString()}', $1, '${res.data.id.toString()}')`, [convertDate(new Date())])
//         return true
//       } else {
//         await pgNeonClient.query(`CREATE TABLE IF NOT EXISTS ErrorPaidToHotel ( id serial PRIMARY KEY,
//           transferRef text, amount text, timestamp text, transactionId text)`)
//         await pgNeonClient.query(`INSERT INTO ErrorPaidToHotel ( transferRef, amount, timestamp, transactionId) VALUES
//         ('${transferRef}', '${amount.toString()}', $1, '${res.data.id.toString()}')`, [convertDate(new Date())])
//         return false
//       }
//     } else {
