    ...
    for (let p = 0; p < rows.length; p++) {
      ...
      for (let i = 0; i < books.length; i++) {
        ...
        let logRows = await client.query(`SELECT * FROM Logs${hId} where type = ? AND date = ? AND message like ?`,
          ['Check in', book.startDate, `%${book.id}%`])
        if (!logRows.length) {
          const diff = +new Date(book.startDate) - +new Date()

          timers[+hId].push(
            setTimeout(async () => {
              await addLog(+hId, 'Check in', `&V&${roomName}&V& Check &in& time for &${book.name}&&-&${book.id}&-&`
                , new Date(book.startDate), 'N/A')
            }, diff <= 5 ? 5 : diff)
          )
        }
        ...
        logRows = await client.query(`SELECT * FROM Logs${hId} where type = ? AND date = ? AND message like ?`,
          ['Check out', book.endDate, `%${book.id}%`])
        if (!logRows.length) {
          const diff = +new Date(book.endDate) - +new Date()

          timers[+hId].push(
            setTimeout(async () => {
              await addLog(+hId, 'Check out', `&V&${roomName}&V& Check &out& time for &${book.name}&&-&${book.id}&-&`
                , new Date(book.endDate), 'N/A')
            }, diff <= 5 ? 5 : diff)
          )
        }
      }
